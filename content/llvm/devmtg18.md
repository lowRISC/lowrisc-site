...
than requiring a specific extension is enabled.
...
Consider the following somewhat contrived example aligns an input value:
...
hex), which does not. Looking at the SelectionDAG dumped after each stage, we 
...
addition and the addition can only affect a small number of bits), so creating 
...
with `aligner.ll` as it's input to verify that the `llvm_unreachable` is hit 
