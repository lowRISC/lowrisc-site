 ...
 * `driver`: Bare metal driver programs used by the first stage boot loaders.
 * `examples`: Some bare metal test programs for FPGA peripherals.
  * `boot`: Bootloader test, also serve as the on-chip boot program used by the FPGA demo.
  * `dram`: Test program for DDR3 RAM interface.
  * `hello`: A simple hello world program use the UART interface.
 ...
 * `script`: Some help Vivado scripts.
...
This make target will automatically process all compiling process from Chisel compilation to bitstream generation. The final bit stream is located at `lowrisc-chip-imp/lowrisc-chip-imp.runs/impl_1/chip_top.bit`. However, in most cases, this bitstream may NOT be th one downloaded to the FPGA. Users can freely change the boot BRAM image after a bitstream is generated.

For installing the USB-JTAG and USB-UART bridge, please see [[Install Xilinx Vivado]] ({{<relref "xilinx.md">}}).
...
This is simple example testing the UART interface.
...
This example execute a read/write check of the DDR3 RAM interface. The check results are constantly printed to UART.
...
This example copies the content of a executable named "`boot`" from a SD card to DDR3 RAM and then reset from DDR3 RAM.

This example also served as the initial boot for booting Linux. It will copy the first stage bootloader from SD to DDR3 RAM.
 ...
 * `boot`: The first stage bootloader for loading Linux kernel.
 * `vmlinux`: The RISC-V Linux kernel.
 * `root.bin`: The ramdisk image.

For generating your own `vmlinux` and `root.bin`, please see [[Compile the RISC-V Linux and the ramdisk `root.bin`]] ({{<relref "linux_compile.md">}}).
...
For generate the first stage bootloader:

    cd $TOP/fpga/board/kc705
    make fsbl
    cp fsbl/fsbl /PATH/TO/SD/boot
